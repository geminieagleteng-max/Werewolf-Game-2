...
export const Lobby = () => {
  ...
  const [playerName, setPlayerName] = useState(() => currentUser?.name || localStorage.getItem('werewolf_player_name') || '玩家一');
        ...
        <div className="w-full max-w-3xl bg-zinc-900/70 border border-zinc-800 rounded-2xl p-8 shadow-sm backdrop-blur-md">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-xs text-zinc-400 mt-1.5">
              創建新房間或輸入代碼加入既有對局 · 支援 Google Identity Services 一鍵登入
            </p>
          </div>
          ...
          {/* Google 快速登入/身分卡片 (醒目橫幅) */}
          <div className="max-w-md mx-auto mb-6 p-4 rounded-2xl bg-gradient-to-r from-blue-950/40 via-zinc-900/90 to-indigo-950/40 border border-blue-500/40 shadow-lg shadow-blue-950/20 flex flex-col sm:flex-row items-center justify-between gap-3">
            <div className="flex items-center gap-3.5 w-full sm:w-auto">
              <div className="relative flex-shrink-0">
                {currentUser?.picture ? (
                  ...
                  <div className="w-12 h-12 rounded-2xl bg-white flex items-center justify-center shadow-md p-1">
                ...
                {isGoogleLinked && (
                  <span className="absolute -bottom-1 -right-1 w-4 h-4 rounded-full bg-white flex items-center justify-center shadow-md p-0.5">
              ...
              <div className="min-w-0 flex-1">
                <div className="text-xs font-bold text-white flex items-center gap-1.5 truncate">
                  <span>{isGoogleLinked ? currentUser?.name : 'Google 帳號登入 / 綁定'}</span>
                  {isGoogleLinked ? (
                    <span className="text-[9px] bg-emerald-950 text-emerald-300 border border-emerald-800 px-1.5 py-0.2 rounded font-medium">
                      Google 驗證
                    </span>
                  ) : (
                ...
                <div className="text-[11px] text-zinc-400 truncate mt-0.5">
                  {isGoogleLinked ? currentUser.email : '登入後同步個人大頭貼、戰績與榮譽成就'}
                </div>
              </div>
            </div>
            ...
            <button
              ...
              className="w-full sm:w-auto px-4 py-2.5 bg-white hover:bg-zinc-100 text-zinc-950 font-bold text-xs rounded-xl shadow-md transition-all cursor-pointer flex items-center justify-center gap-2 flex-shrink-0"
            >
              ...
              <span>{isGoogleLinked ? '管理 Google 帳號' : 'Google 一鍵登入 / 綁定'}</span>
            </button>
          </div>
          ...
          return (
            ...
            >
              ...
              <div className="text-center my-auto flex flex-col items-center">
                <div className="relative mb-1">
                  {player.avatar ? (
                    <img
                      src={player.avatar}
                      alt={player.name}
                      className="w-8 h-8 rounded-full object-cover border border-amber-500/50 shadow-xs"
                    />
                  ) : (
                  ...
                  {player.authProvider === 'GOOGLE' && (
                    <span className="absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full bg-white flex items-center justify-center shadow-xs p-0.5">
                ...
                <div className="text-xs font-bold text-zinc-200 truncate max-w-[110px]">
                  {player.name} {isMe && '(我)'}
                </div>
                {isMe && equippedTitle && (
                  <span className="mt-0.5 text-[9px] font-bold text-amber-300 bg-amber-950/80 px-1.5 py-0.2 rounded-full border border-amber-800/60 max-w-[100px] truncate">
                    👑 【{equippedTitle}】
                  </span>
                )}
              </div>
