      ...
      <header className="flex flex-wrap items-center justify-between gap-4 px-6 py-3 bg-zinc-950 border-b border-zinc-800/80 text-zinc-100">
        ...
        <div className="flex items-center gap-2 sm:gap-2.5">
          ...
          <button
            ...
            className="flex items-center gap-2 px-2 py-1 bg-zinc-900 hover:bg-zinc-800 border border-zinc-800 hover:border-zinc-700 rounded-lg transition-all cursor-pointer shadow-sm group"
            title={isGoogleLinked ? `Google 帳號已綁定: ${currentUser?.name} (${currentUser?.email})` : 'Google 帳號登入 / 訪客檔案'}
          >
            <div className="relative flex-shrink-0">
              {currentUser?.picture ? (
                ...
                <div className="w-5 h-5 rounded-md bg-zinc-800 border border-zinc-700 flex items-center justify-center text-[10px] text-zinc-400">
                  👤
                </div>
              )}
              {isGoogleLinked && (
            ...
            <span className="text-[11px] font-medium text-zinc-300 hidden md:inline max-w-[65px] truncate">
              {currentUser?.name || '訪客'}
            </span>
          </button>
