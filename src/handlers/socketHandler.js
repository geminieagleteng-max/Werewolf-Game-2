...
function setupSocketHandlers(io) {
  io.on('connection', (socket) => {
    ...
    // 預言家查驗
    socket.on(SOCKET_EVENTS.ACTION.SEER_CHECK, ({ targetId }) => {
      const { room, player } = roomManager.findPlayerBySocketId(socket.id);
      if (!room || !room.game || room.game.phase !== GAME_PHASES.NIGHT_SEER) return;

      const checkRes = room.game.handleSeerCheck(player.id, targetId);
      if (!checkRes.success) {
      ...
      socket.emit(SOCKET_EVENTS.GAME.SYSTEM_MSG, {
        message: `🔮 查驗結果：【${checkRes.result.seatNumber}號 ${checkRes.result.targetName}】身分為 ${checkRes.result.factionName}`,
      });
    });
